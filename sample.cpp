...
public:
    void parse(const uint8_t* data, size_t len) override {
        if (len < 14) {
            throw std::runtime_error("유효하지 않은 이더넷 프레임: 길이가 너무 짧습니다.");
        }
        std::memcpy(destMac.data(), data, 6);
        std::memcpy(srcMac.data(), data + 6, 6);
...
public:
    void parse(const uint8_t* data, size_t len) override {
        if (len < 20) {
            throw std::runtime_error("유효하지 않은 IPv4 패킷: 길이가 너무 짧습니다.");
        }
        version = (data[0] >> 4) & 0xF;
        ihl = data[0] & 0xF;
        if (ihl < 5) {
            throw std::runtime_error("유효하지 않은 IPv4 헤더 길이");
        }
        tos = data[1];
        totalLength = ntohs(*reinterpret_cast<const uint16_t*>(data + 2));
        identification = ntohs(*reinterpret_cast<const uint16_t*>(data + 4));
        flagsAndFragmentOffset = ntohs(*reinterpret_cast<const uint16_t*>(data + 6));
        ttl = data[8];
        protocol = data[9];
        headerChecksum = ntohs(*reinterpret_cast<const uint16_t*>(data + 10));
        srcIP = ntohl(*reinterpret_cast<const uint32_t*>(data + 12));
        destIP = ntohl(*reinterpret_cast<const uint32_t*>(data + 16));
    }
